...
const VehicleCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSuccess('');
    try {
      await api.post('/vehicle/new', form);
      setSuccess('Veículo cadastrado com sucesso!');
      setTimeout(() => navigate('/vehicles'), 1200);
    } catch (err) {
      setError(
        err.response?.data?.message || 'Erro ao cadastrar veículo.'
      );
    }
  };
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex flex-col flex-1 bg-blue-50 dark:bg-gray-700">
        ...
        <div className="bg-blue-100 text-blue-700 dark:bg-gray-600 dark:text-blue-100 px-6 py-6 shadow-sm flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold">Formulário de Veículos</h1>
        </div>

        <div className="mt-10 dark:bg-gray-800 p-8 m-10 rounded shadow rounded-lg">
          <h2 className="text-2xl font-bold mb-6 text-center dark:text-blue-50">Novo Veículo</h2>
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block mb-1 text-sm font-bold text-gray-800 dark:text-blue-50">Placa</label>
              <input
                type="text"
                name="plate"
                value={form.plate}
            ...
            <div className="mb-4">
              <label className="block mb-1 text-sm font-bold text-gray-800 dark:text-blue-50">Modelo</label>
              <input
                type="text"
                name="model"
                value={form.model}
            ...
            <div className="mb-4">
              <label className="block mb-1 text-sm font-bold text-gray-800 dark:text-blue-50">Marca</label>
              <input
                type="text"
                name="mark"
                value={form.mark}
